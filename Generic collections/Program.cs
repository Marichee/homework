...
using Generetic_collections;
using Generetic_collections.Models;
namespace Generic_collections
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Person Jerry = new Person("Jerry", "Tompson", 78, 23, Genre.Rock);
            Person Stefan = new Person("Stefan", "Stefanoski", 28, 24, Genre.Techno);
            Person Maria = new Person("Maria", "Campbel", 43, 29, Genre.Classical);
            Person Jane = new Person("Jane", "Doe", 28, 45, Genre.Techno);
            Jerry.FavoriteSongs = Songs;
            Stefan.FavoriteSongs = Songs;
            Maria.FavoriteSongs = Songs;
            Jane.FavoriteSongs = Songs;
            FansArray.Add(Jerry);
            FansArray.Add(Stefan);
            FansArray.Add(Maria);
            FansArray.Add(Jane);
            Jerry.FavoriteSongs = Songs.Where(song => song.Title.StartsWith("B")).ToList();
            Console.WriteLine($"{Jerry.Firstname} favorite songs which starts with B:");
            foreach (var song in Jerry.FavoriteSongs)
            {
                Console.WriteLine($"{song.Title}");
            }
            Maria.FavoriteSongs = Songs.Where(song => song.Length >= 360).ToList();
            Console.WriteLine($"{Maria.Firstname} favorite songs with length longer than 6 min:");
            foreach (var song in Maria.FavoriteSongs)
            {
                Console.WriteLine($"TITLE:{song.Title} LENGTH:{song.Length}");
            }
            Jane.FavoriteSongs = Songs.Where(genre => genre.Genre == Genre.Rock).ToList();
            Console.WriteLine($"{Jane.Firstname} favorite songs that are of genre Rock:");
            foreach (var song in Jane.FavoriteSongs)
            {
            ...
            Console.WriteLine($"{Stefan.Firstname} favorite songs shorter than 3 min and of genre Hip Hop:");
            Stefan.GetFavSongs();
            var personsThatHave4OrMoreSongs = FansArray.Where(persons => persons.FavoriteSongs.Count <= 4).ToList();
            Console.WriteLine("Persons that have 4 or more music:");
            foreach (var person in personsThatHave4OrMoreSongs)
            {
